...
class RichPrinter:
    ...
    def display_dataframe_as_table(
        ...
        Raises:
            ValueError: If the provided `*columns` are actually empty.
        """
        if not len(data_frame):
            raise ValueError("DataFrame cannot be empty.")

        if show_header and not len(data_frame.columns):
            raise ValueError("DataFrame must have columns when `show_header` is True.")

        table: Table = Table(show_header=show_header, header_style=header_style)
